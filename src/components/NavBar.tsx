  ...
  return (
    <nav
      ...
      style={{
        ...
        minHeight: collapsed ? 70 : 170,
        height: collapsed ? 70 : 170,
      }}
    >
      {/* Branding (unchanged, always visible on desktop) */}
      {!collapsed && (
      ...
      {/* Hamburger menu for mobile: now always shown on mobile (not just expanded) */}
      <button
        className={cn(
      ...
      {/* Mobile: Dropdown nav panel - shown below hamburger icon on any state */}
      {showMobileMenu && (
        <ul
          className={cn(
            "sm:hidden flex flex-col items-stretch absolute z-[70]",
            "rounded-xl shadow-2xl border border-pink-600 bg-black/95 min-w-[160px] overflow-hidden animate-fade-in"
          )}
          style={{
            right: 12,
            top: 62, // Always position just below hamburger
          }}
        >
      ...
      {/* Desktop nav panel - never visible on mobile */}
      <ul
        className={cn(
          ...
          "hidden sm:flex" // Only desktop/tablet
        )}
        style={{
          position: "absolute",
          zIndex: 30,
      ...
      >
        {sections.map(({ id, label, icon: Icon }) => (
          <li key={id}>
            ...
            >
              <Icon size={collapsed ? 22 : 20} className="inline -mt-1 mr-1 text-portfolio-gradient-from" color="#ff3796" />
